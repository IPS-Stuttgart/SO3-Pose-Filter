...
class MotionStratifiedPrivateEvalTests(unittest.TestCase):
    ...
    def test_motion_bin_segment_diagnostics_are_written_before_no_runs_error(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            output_dir = root / "motion_stratified_eval"
            with self.assertRaisesRegex(ValueError, "no benchmark-loadable motion bins"):
                run_motion_stratified_private_accad_eval(
                    {
                        "dataset_name": "ToyKIT",
                        "source_data_root": str(raw_dir),
            ...
            copied = list((output_dir / "motion_bin_segments" / "high_motion").glob("*.npz"))
